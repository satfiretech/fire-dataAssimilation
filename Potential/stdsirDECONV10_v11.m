function [KeepFireDetect x_ prediction Keep_Q Keep_Inn Keep_S stResidual z cycle_out_scope cycle_reported estimation Keep_Ra predictedObservation t xf FEATURE1 y_predictedKeep] = stdsirDECONV10(TESTINGCYCLE_DATA,NUMBER_STANDARD_DEVIATION,timestamp,n,x_,prediction,Keep_Q,Keep_Inn,Keep_S,KeepFireDetect,DECONV,restored,Keep_Ra,NUMBER_PARTICLE_SIR, predictedObservation, threshold, FEATURE1, RESIDUAL,  y_predictedKeep)
%function [KeepFireDetect x_ prediction Keep_Q Keep_Inn Keep_S stResidual z cycle_out_scope cycle_reported estimation Keep_R predictedObservation t xf] = stdsirDECONV10(TESTINGCYCLE_DATA,NUMBER_STANDARD_DEVIATION,timestamp,n,x_,prediction,Keep_Q,Keep_Inn,Keep_S,KeepFireDetect,DECONV,restored,Keep_R,NUMBER_PARTICLE_SIR, predictedObservation, threshold)
...
if timestamp==1
    Qi = zeros(NUMBER_ROW,NUMBER_COLUMN); %If Q is time varying
end
...
for jN = 1:NUMBER_ROW
    for iN = 1:NUMBER_COLUMN
        ...
        w1(jN,iN) = TESTINGCYCLE_DATA(jN,iN).w(n(jN,iN)); %1
        w2(jN,iN) = TESTINGCYCLE_DATA(jN,iN).w2(n(jN,iN)); %1
        To(jN,iN) = TESTINGCYCLE_DATA(jN,iN).To(n(jN,iN)); %1
        Ta(jN,iN) = TESTINGCYCLE_DATA(jN,iN).Ta(n(jN,iN)); %1
        tm(jN,iN) = TESTINGCYCLE_DATA(jN,iN).tm(n(jN,iN)); %1
        ts(jN,iN) = TESTINGCYCLE_DATA(jN,iN).ts(n(jN,iN)); %1
        %dT(jN,iN) = TESTINGCYCLE_DATA(jN,iN).dT(n(jN,iN));
        k(jN,iN) = TESTINGCYCLE_DATA(jN,iN).k(n(jN,iN)); %1
        %if timestamp==1
        ...
        %else
        %    n = n + 1;
        %Q(jN,iN) = TESTINGCYCLE_DATA(jN,iN).Q(n(jN,iN)); %1
...
xf = zeros(6,NUMBER_ROW*NUMBER_COLUMN,Ns); %xf is filtered samples, it will keep all filtered distribution, each distribution on a each pixels
...
% %RESTORE MISSING DATA FOR Q = Q IS FOUND BEFORE HAND AND IT HAS NO NAN
...
process_noise = normalizedState(Q,randn(size(xf))); %[Number of variables x Number of pixels]
...
if timestamp==1
    %Input xi=mean over a number of days
...
%inn = z - prediction(:,:,timestamp); %Inn is the residual
predictedObservation(:,:,timestamp) = observationFunction(prediction(:,:,timestamp),z,t);
inn = z - predictedObservation(:,:,timestamp); %observationFunction(prediction(:,:,timestamp),z,t);
...
%Define H as the Jacobian: CHANGE HERE
%H = ones(1,7);
%HPH with H as a linear observation operator and P determined as 
%-----
% H = ones(2,6); %Instead call linearObservationOperator with x_
% S = backgroundCovariance(x_ - extend(prediction(:,:,timestamp),Ns),H,R) + R;
%-----
Rx = reshape(R,1,size(R,1)*size(R,2));
Rc = reshape(R1c,1,size(R1c,1)*size(R1c,2));
for i = 1:size(R,1)*size(R,2)
    Ra(1,1+(i-1)*2:2+(i-1)*2) = [Rx(i) 0]; 
    Ra(2,1+(i-1)*2:2+(i-1)*2) = [0 Rc(i)];
end

z_extended = extend(z,Ns);
z_predicted = observationFunction(x_,z_extended,t);
              %observationFunction(x_,z_extended,t)


c_extended = extend(c,Ns);
c_predicted = thermalsunriseFunction(x_,c_extended,t,extend(tHorizon,Ns));

HX = z_predicted - extend(mean(z_predicted,3),Ns);
HXc = c_predicted - extend(mean(c_predicted,3),Ns);
...
for i=1:size(HXa,2) %Number of pixels
    ...
    HPH = [HPH squeeze(HXa(:,i,:))*(squeeze(HXa(:,i,:))).'/(Ns-1)]; %2 x (2 . Pixels)
end
...
%Keep_R(1:size(t,1),1:size(t,2),timestamp) = R; %Keep the measurement covariance
Keep_Ra(1:2,1:size(t,1)*size(t,2)*2,timestamp) = Ra; %Keep the measurement covariance
%Keep_Q(1:size(t,1),1:size(t,2),timestamp) = Q;
clear Q
...
%Get fire detection result at launch of the SIR
if timestamp==1
    if RESIDUAL == 0
        [FIREFLAG FEATURE1 y_predictedM] = startTimeFireDetect(z, Keep_Ra, x_, t, threshold, RESIDUAL);
...
%alpha = z_extended - observationFunction(x_,z_extended,t);
alpha = z_extended - z_predicted;

alphac = c_extended - c_predicted; %50 x 50 x Ns



%To+Ta.*cos(pi./w1.*(t-tm))
%ASSUMED THAT THE LAUNCH TIME IS AT THE RISING EDGE OF DTC  
%====


alphaAll = zeros(2,size(alpha,1)*size(alpha,2),Ns);
alphaAll(1,:,:) = reshape(alpha,1,size(alpha,1)*size(alpha,2),Ns);
alphaAll(2,:,:) = reshape(alphac,1,size(alphac,1)*size(alphac,2),Ns);

KeepFireDetectx = reshape(KeepFireDetect(:,:,timestamp),1,size(KeepFireDetect,1)*size(KeepFireDetect,2));
for i=1:size(alpha,1)*size(alpha,2) %Nbre of pixels
    %increment(i,:,:) = extend(Gf(i,:),Ne).*alpha;
    if KeepFireDetectx(i)==0  %& isinf(S(1))==0
        for j = 1:Ns
            LIKELIHOOD_F(:,i,j) = exp(-0.5*(alphaAll(:,i,j)'*(Ra(1:2,1+(i-1)*2:2+(i-1)*2)\[1 0;0 1])*alphaAll(:,i,j)));
            %LIKELIHOOD_F(:,i,j) = exp(-0.5*(alphaAll(:,i,j)'*inv(Ra(1:2,1+(i-1)*2:2+(i-1)*2))*alphaAll(:,i,j)));
            %         increment(1:6,i,j) = Gf(1:6,1+(i-1)*2:2+(i-1)*2)*alphaAll(:,i,j);
        end
    else
        for j = 1:Ns
            LIKELIHOOD_F(:,i,j) = exp(-0.5*(alphaAll(:,i,j)'*zeros(2,2)*alphaAll(:,i,j)));
            %         increment(1:6,i,j) = Gf(1:6,1+(i-1)*2:2+(i-1)*2)*alphaAll(:,i,j);
        end
    end
end
...
for pixel = 1:NUMBER_ROW * NUMBER_COLUMN      
        cdf_q = cumsum(q(:,pixel,:)); cdf_q = cdf_q(:);%Construct CDF
        i = 1;
...
% % % % % % % % % % % % % % %     Pr(keep_t_ts==0) =  To(keep_t_ts==0)+ (keep_xf(keep_t_ts==0)-To(keep_t_ts==0)).*exp(-1/4./k(keep_t_ts==0)) +keep_process_noise(keep_t_ts==0);
    ...
    %x_ = xf + process_noise./extend(MULTIPLIER,Ns); %+ process_noise/8;
    x_ = xf + process_noise./extend(sqrt(2)*erfinv(1-1/numel(xf))*sqrt(MULTIPLIER),Ns); 
